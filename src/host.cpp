...
void init(std::vector<int, aligned_allocator<int>>& ClauseList, std::vector<int, aligned_allocator<int>>& VarsOccList, 
		...
		while (!fileDIMACS.eof()) {
			...
			if ((p > '0' && p <= '9') || p == '-' || p == ' ') {
				...
				while (true) {
					...
					for (int i = 0; i < r; i++) {
						if (VarsOccList[GETPOS(nextLit) * r * DSIZE + i] == 0) {
							VarsOccList[GETPOS(nextLit) * r * DSIZE + i] = clauseCounted;
	...
	for (int v = 0; v < numVars * 2; v++) {
		...
		for (int i = 0; i < rlen; i++) {
			int loc = VarsOccList[DSIZE * r * v + i]; 
...
int main(int argc, char** argv) {
    ...
    std::vector<int, aligned_allocator<int>> ClauseList(MAXNCLS * MAXK, 0);
	std::vector<int, aligned_allocator<int>> VarsOccList(MAXNLIT * MAXR * DSIZE, 0);
	...
	double rtime = exec_time*1e-9;
	double Mfps = (double)flipcnt[0] / rtime;
	double MaxMfps = maxFlip / rtime; 
	...
	if(flipcnt[0] > 0){
		...
		std::cout << "Time: " << exec_time*1e-9 << ", flips: " << flipcnt[0] << ", Mfps: " << Mfps << std::endl;
	}
	else{
		std::cout << "Solver could not find a solution. | ";
		std::cout << "Time: " << exec_time*1e-9 << ", flips: " << maxFlip << ", Mfps: " << MaxMfps << std::endl;
	}
