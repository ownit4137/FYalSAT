...
void forcsim(hls::vector<length, SDSIZE> ol_len_off[], hls::vector<clength, SDSIZE> cls_len_off[],
	...
	VarsOccList_c = new hls::vector<int, DSIZE>[MAXNLIT * MAXR];
	// tl_XORed_partd_c = new cls*[MAXNCLS / DSIZE + 1]; 
	// for (int i = 0; i < MAXNCLS / DSIZE + 1; i++) {
    // 	tl_XORed_partd_c[i] = new cls[DSIZE];
	// }

	// cost_partd_c = new cost*[MAXNCLS / DSIZE + 1]; 
	// for (int i = 0; i < MAXNCLS / DSIZE + 1; i++) {
    // 	cost_partd_c[i] = new cost[DSIZE];
	// }

	// // cls UCB_partd_c[UCBSIZE / DSIZE + 1][DSIZE];
	// // ucbidx posInUCB_c[MAXNCLS / DSIZE + 1][DSIZE];	

	// UCB_partd_c = new cls*[UCBSIZE / DSIZE + 1]; 
	// for (int i = 0; i < UCBSIZE / DSIZE + 1; i++) {
    // 	UCB_partd_c[i] = new cls[DSIZE];
	// }

	// posInUCB_c = new ucbidx*[MAXNCLS / DSIZE + 1]; 
	// for (int i = 0; i < MAXNCLS / DSIZE + 1; i++) {
    // 	posInUCB_c[i] = new ucbidx[DSIZE];
	// }

	// // bscore bsArr_c[MAXNVAR + 1];		// break score
	// // bscore bs_dif_partd_c[MAXNVAR + 1][DSIZE];

	// bs_dif_partd_c = new bscore*[MAXNVAR + 1]; 
	// for (int i = 0; i < MAXNVAR + 1; i++) {
    // 	bs_dif_partd_c[i] = new bscore[DSIZE];
	// }

	// bsArr_c = new bscore[MAXNVAR + 1]; 

	// // clength cls_len_c[MAXNCLS + 1];
	// // length ol_len_c[MAXNLIT];
	
	// cls_len_c = new clength[MAXNCLS + 1]; 
	// ol_len_c = new length[MAXNLIT]; 
...
bool verify() {
	...
	delete[] VarsOccList_c;
	
	// for (int i = 0; i < MAXNCLS / DSIZE + 1; i++) {
    // 	delete[] tl_XORed_partd_c[i];
	// }
	// delete[] tl_XORed_partd_c; 

	 
	// for (int i = 0; i < MAXNCLS / DSIZE + 1; i++) {
    // 	delete[] cost_partd_c[i];
	// }
	// delete[] cost_partd_c;
	
	// for (int i = 0; i < UCBSIZE / DSIZE + 1; i++) {
    // 	delete[] UCB_partd_c[i];
	// }
	// delete[] UCB_partd_c; 
	...
	// for (int i = 0; i < MAXNVAR + 1; i++) {
    // 	delete[] bs_dif_partd_c[i];
	// }

	// delete[] bs_dif_partd_c; 
