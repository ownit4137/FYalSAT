...
void mod_break_2() {
	...
	lookup_break: for (int i = 0; i < clen; i++) {
		...
		// std::cout << vidx << " " << bv << " " << p << " " << sumProb << std::endl;
	...
	// std::cout << var_flip << "seelcted" << std::endl;
...
void mod_main(int numVars, int numClauses, int s, unsigned long long flipcnt[]) {
	...
	flip: for (unsigned long long f = 0; f <= maxFlip; f++) {
		...
		// for (int i = 0; i < 16; i++) {
		// 	for (int j = 0; j < UCB_partd_len_c[i]; j++) {
		// 		std::cout << UCB_partd_c[j][i] << " ";
		// 	}
